...
const SEVERITY_COLORS = {
  low: { color: '#27ae60', label: 'risk.severity.low' },
  medium: { color: '#f39c12', label: 'risk.severity.medium' },
  high: { color: '#e67e22', label: 'risk.severity.high' },
  critical: { color: '#e74c3c', label: 'risk.severity.critical' },
}
...
export default function Tab3_Myths() {
  ...
  return (
    <div className="space-y-5">
      {/* Myths flip cards */}
      <div>
        <h3 className="font-bold mb-2 text-lg" style={{ color: '#1a1a1a' }}>💡 {t('tab3.title')}</h3>
        <p className="text-sm mb-4" style={{ color: '#888' }}>👆 {t('tab3.clickToFlip')}</p>
                  ...
                  <div className="flip-card-inner relative" style={{ height: '100%' }}>
                    ...
                    <div className="flip-card-front absolute inset-0 p-5 rounded-2xl flex flex-col cursor-pointer" style={{ background: 'rgba(231,76,60,0.07)', border: '1.5px solid rgba(231,76,60,0.25)', boxShadow: '0 2px 12px rgba(231,76,60,0.08)' }}>
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        <span className="text-xs font-bold uppercase tracking-widest" style={{ color: '#e74c3c' }}>{t('tab3.mythLabel')}</span>
                      </div>
                      <p className="text-sm leading-relaxed flex-1 flex items-center" style={{ color: '#374151' }}>{t(myth.mythKey)}</p>
                    </div>
                    {/* Back */}
                    <div className="flip-card-back absolute inset-0 p-5 rounded-2xl flex flex-col cursor-pointer" style={{ background: 'rgba(39,174,96,0.07)', border: '1.5px solid rgba(39,174,96,0.25)', boxShadow: '0 2px 12px rgba(39,174,96,0.08)' }}>
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        <span className="text-xs font-bold uppercase tracking-widest" style={{ color: '#27ae60' }}>{t('tab3.factLabel')}</span>
                      </div>
                      <p className="text-sm leading-relaxed flex-1 flex items-center" style={{ color: '#374151' }}>{t(myth.factKey)}</p>
                    </div>
                  </div>
                </div>
              </motion.div>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }} className="glass-card p-5">
        <h3 className="font-bold mb-3" style={{ color: '#1a1a1a' }}>📋 {t('tab3.indications.title')}</h3>
        <ul className="space-y-2">
          {(t('tab3.indications.items', { returnObjects: true }) as string[]).map((item, i) => (
            <li key={i} className="flex items-start gap-2 text-sm" style={{ color: '#4b5563' }}>
              <span className="flex-shrink-0 mt-0.5 font-bold" style={{ color: '#c0392b' }}>▸</span>
              {item}
            </li>
          ))}
        </ul>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.6 }} className="glass-card p-5">
        <h3 className="font-bold mb-1" style={{ color: '#1a1a1a' }}>{t('tab3.risks.title')}</h3>
        <p className="text-xs mb-4" style={{ color: '#888' }}>{t('tab3.risks.subtitle')}</p>
            ...
            return (
              <div key={r.id} className="flex items-center justify-between p-2.5 rounded-xl" style={{ background: 'rgba(0,0,0,0.025)' }}>
                <div className="flex items-center gap-2 flex-1 min-w-0">
                  ...
                  <span className="text-sm truncate" style={{ color: '#374151' }}>{t(r.labelKey)}</span>
                </div>
                <div className="flex items-center gap-2 flex-shrink-0 ml-2">
                  <span className="text-xs" style={{ color: '#9ca3af' }}>{r.rate}</span>
